...
int main() {
    ...
    Node *beforefirst = new Node(5);
    beforefirst->next = head;
    head->prev = beforefirst;

    printLL(); // 5 10 20 30

    // new node between second and third
    Node *aftersecond = new Node(25);
    aftersecond->next = third;
    aftersecond->prev = second;

    second->next = aftersecond;
    third->prev = aftersecond;

    printLL(); // 5 10 20 25 30

    // deletion
    aftersecond = 0; // let's say you lost the value, delete the node that comes after 20
